...
public class MyIntentService extends IntentService {
    ...
    private void handleActionMusic2() {
        if(mediaPlayer != null) mediaPlayer.stop();
        // 根据音乐资源文件创建MediaPlayer对象 设置循环播放属性 开始播放
        mediaPlayer = MediaPlayer.create(this, R.raw.bit);
        mediaPlayer.setLooping(true);
        mediaPlayer.start();
    }
    private void handleActionMusic3() {
        if(mediaPlayer != null) mediaPlayer.stop();
        // 根据音乐资源文件创建MediaPlayer对象 设置循环播放属性 开始播放
        mediaPlayer = MediaPlayer.create(this, R.raw.lib_read);
        mediaPlayer.setLooping(true);
        mediaPlayer.start();
    }
    private void handleActionMusic4() {
        if(mediaPlayer != null) mediaPlayer.stop();
        // 根据音乐资源文件创建MediaPlayer对象 设置循环播放属性 开始播放
        mediaPlayer = MediaPlayer.create(this, R.raw.read);
        mediaPlayer.setLooping(true);
        mediaPlayer.start();
    }
    ...
    private void handleActionMusic6() {
        if(mediaPlayer != null) mediaPlayer.stop();
        // 根据音乐资源文件创建MediaPlayer对象 设置循环播放属性 开始播放
        mediaPlayer = MediaPlayer.create(this, R.raw.ourtime);
        mediaPlayer.setLooping(true);
        mediaPlayer.start();
    }
